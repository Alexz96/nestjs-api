...
export class OrdersService implements OnModuleInit {
  ...
  /**
   * * Neste metodo passamos a sessao para que tenhamos acesso a tudo que foi criado nela
   * * enquanto executamos a transacao
   * ? devemos ter cuidado com transacoes concorrentes e os bancos NoSQL nao utilizam a trava por padrao
   * ? precisamos entao validar a versao do documento, se a em memoria eh igual a do banco
   */
  async createTrade(dto: CreateTradeDto) {
    ...
    try {
      ...
      await order.save({ session }); // ? em cada save que eh checado automaticamente a versao do documento na memoria com a do banco
